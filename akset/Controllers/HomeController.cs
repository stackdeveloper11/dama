...
namespace akset.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        // [CompressFilter]
        // [OutputCache(Duration = 5, VaryByParam = "departure;arrival;flightType;pax;departureDate;classman;ifdirect;IfRefundablesOnly;discity;aiscity;depc;arrc;returnDate")]
        public JsonResult urungetir(int id)
        {
            List<urun> ddd = new List<urun>();
            ddd = db.uruns.Where(a => a.KategoriId == id).ToList();
            List<urunletable> ggg = new List<urunletable>();
            foreach (var item in ddd)
            {
                urunletable urn = new urunletable();
                urn.Id = item.Id;
                urn.tarih = item.tarih;
                urn.resim = item.resim;
                urn.adi = item.urunadi;
                urn.baslik = item.urunbaslik;
                urn.urunkodu = item.UrunKodu;
                urn.eskifiyat =string.IsNullOrEmpty(item.eskifiyat)? "0": (Convert.ToDecimal(item.fiyat) - (Convert.ToDecimal(item.fiyat) * Convert.ToDecimal(item.eskifiyat) / 100)).ToString();
                urn.fiyat = item.fiyat;
                urn.link = item.link;
                urn.satistami = item.satistami;
                string v1 = "";
                string v2 = "";
                if (Directory.Exists(Server.MapPath("~/urunresimleri/" + item.resim + "/")))
                {
                    var files1 = Directory.GetFiles(Server.MapPath("~/urunresimleri/" + item.resim + "/"), "*.*");
                    v1 = Path.GetFileName(files1.FirstOrDefault());
                   // v2 = Path.GetFileName(files1.LastOrDefault());
                    foreach (var fil in files1)
                    {
                        if (Path.GetFileName(fil).Contains("v1-"))
                        {
                            v1 = Path.GetFileName(fil);
                        }
                     
                        if (Path.GetFileName(fil).Contains("v2-"))
                        {
                            v2 = Path.GetFileName(fil);
                        }
                ...
                List<Areas.Admin.Controllers.filadtli> flt = new List<Areas.Admin.Controllers.filadtli>();

                foreach (var itemx in _urun.ToList())
                {
                    Areas.Admin.Controllers.filadtli dddd = new Areas.Admin.Controllers.filadtli();
                    dddd.adi = itemx.filitre.adi;
                    dddd.adetlimi = itemx.filitre.adetlimi;
                    dddd.Id = itemx.filitreId;
                    List<Areas.Admin.Controllers.ozladtli> gg = new List<Areas.Admin.Controllers.ozladtli>();
                    foreach (var itemi in itemx.filitre.ozelliks)
                    {
                        Areas.Admin.Controllers.ozladtli ff = new Areas.Admin.Controllers.ozladtli();
                        ff.Id = itemi.Id;
                        ff.ozellik = itemi.adi;
                        ff.adet = db.urunadetozelliks.Where(a => a.ozellikId == itemi.Id).FirstOrDefault() != null ? db.urunadetozelliks.Where(a => a.ozellikId == itemi.Id).FirstOrDefault().adet : 0;
                        ff.durum = db.urunadetozelliks.Where(a => a.ozellikId == itemi.Id).FirstOrDefault() != null ? db.urunadetozelliks.Where(a => a.ozellikId == itemi.Id).FirstOrDefault().durum : "false";
                        gg.Add(ff);
                    }
                    dddd.ozls = gg;
                    flt.Add(dddd);
                }
                urn.urunlefiltres = flt;
                ggg.Add(urn);
            }
            var jsonResult = Json(ggg.OrderByDescending(a=>a.tarih), JsonRequestBehavior.AllowGet);
            jsonResult.MaxJsonLength = int.MaxValue;
            return jsonResult;
        }
        public ActionResult elfinderi()
        {
            return View();
        }
        ...
        {
            return View(db.Vitrinlers.OrderBy(a => a.aktifmi).ToList());
        }
        public JsonResult aragetir()
        {
            string bsl = "";
            if (db.Kategoris.OrderBy(r => Guid.NewGuid()).FirstOrDefault()!=null)
            {
                bsl = db.Kategoris.OrderBy(r => Guid.NewGuid()).FirstOrDefault().baslik;
            }
            return base.Json(bsl, JsonRequestBehavior.AllowGet);
        }
        public static Bitmap WatermarkImage(Bitmap image, Bitmap watermark,string kod)
        {
            using (Graphics imageGraphics = Graphics.FromImage(image))
            {
                ...
                int x = 10;
                int y = (image.Height) - 110;
                imageGraphics.DrawImage(watermark, x, y, watermark.Width, watermark.Height);

            }

            PointF firstLocation = new PointF(80f, image.Height - 75);
            PointF secondLocation = new PointF(60f, image.Height - 45);

            using (Graphics graphics = Graphics.FromImage(image))
            {
                using (Font arialFont = new Font("Arial", 20))
                {
                    graphics.DrawString("Kod", arialFont, Brushes.Black, firstLocation);
                    graphics.DrawString(kod, arialFont, Brushes.Black, secondLocation);
                }
            }
            return image;
        }
        ...
        //    return newImage;
        //}
        public FileResult Get(string yol,string kod)
        {
            string imageFilePath = Server.MapPath(yol);
            Bitmap bitmap = (Bitmap)Image.FromFile(imageFilePath);
            Image img = bitmap;// ResizeImage(bitmap, new Size(350, 500),true);
            Bitmap dd = new Bitmap(img);
            Bitmap watermark = new Bitmap(Server.MapPath("~/") + "download.png");
            Bitmap ggg = WatermarkImage(dd, watermark,kod); 
           
            var ms = new MemoryStream();
            ggg.Save(ms, ImageFormat.Jpeg);
            ms.Position = 0;
            var result = new FileStreamResult(ms, "image/jpeg");
            return result;
        }
        public ActionResult kayitdogrula(string kod)
        {
            if (kod != "OK" && kod != "HATA")
            {
                var user = db.Users.Where(a => a.Id == kod).FirstOrDefault();
                ViewBag.Id = user.Id;
            }
            else
            {
        ...
        public ActionResult smsdogrula(string kod)
        {
            if (kod != "OK" && kod != "HATA")
            {
                var user = db.Users.Where(a => a.Id == kod).FirstOrDefault();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult smsdogrula(string kod, string Id)
        {

            var user = db.Users.Where(a => a.Id == Id).FirstOrDefault();
            var uri = new Uri(Request.Url.ToString());
            var authority = uri.GetLeftPart(UriPartial.Authority);
            if (user != null && user.smskod == kod)
            {
                string sfr = helper.Genel.RandomString(6);
                user.PhoneNumberConfirmed = true;

                db.Entry(user).State = EntityState.Modified;
                db.SaveChanges();
                // AccountController gg = new AccountController();
                // gg.SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: true);
                helper.Genel.smsyolla2(user.Soyad, "Hesabiniz Basariyla Dogrulanmistir. Iyi alisverisler dileriz.", user.PhoneNumber);
                return Redirect(authority + "/giris-yap");
            }
            return Redirect(authority + "/home/smsdogrula/" + "HATA");
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult kayitdogrula(string kod, string Id)
        {
            ...
            if (user != null && user.smskod == kod)
            {
                ...
                data.aksetDB _ctx = new data.aksetDB();
                UserManager<User> userManager = new UserManager<User>(new UserStore<User>(_ctx));
                Guid g4 = Guid.NewGuid();
                user.SecurityStamp = g4.ToString();
                user.PasswordHash = userManager.PasswordHasher.HashPassword(sfr);
                db.Entry(user).State = EntityState.Modified;
                db.SaveChanges();
                // AccountController gg = new AccountController();
                // gg.SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: true);
                helper.Genel.smsyolla2(user.Soyad, "Sifreniz Basariyla Degistirilmistir. Yeni sifreniz: " + sfr, user.PhoneNumber);
                return Redirect(authority + "/giris-yap");
            }
            return Redirect(authority + "/kayit-dogrula/" + "HATA");
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult adresdeg(string adr, string phn)
        {
            string fff = "jjjj";
            string ffffF = "jjjj";
            var id = User.Identity.GetUserId();
            var usr = db.Users.Where(a => a.Id == id).FirstOrDefault();
            usr.adrestelefon = phn;
            usr.adres = adr;
        ...
        [OutputCache(Duration = 120, VaryByParam = "none")]
        public ActionResult category(string ilk)
        {
            int id = 0;
            foreach (var item in db.Kategoris.ToList())
            {
                if (akset.helper.Genel.URLFriendly(item.baslik) == ilk)
                {
                    id = item.Id;
                }
            }
            ...
            return View(ggg.OrderByDescending(a=>a.tarih).ToList());
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult profil(string ad, string soyad, string phn, string birthday, string gender)
        {
            var id = User.Identity.GetUserId();
            var usr = db.Users.Where(a => a.Id == id).FirstOrDefault();
            usr.Ad = ad;
            usr.Soyad = soyad;
            usr.PhoneNumber = phn;
            usr.DogumTarihi = Convert.ToDateTime(birthday);
            usr.cinsiyet = gender;
            string dd = "";
            try
            {
            ...
            catch (DbEntityValidationException e)
            {
                ...
                {
                    dd += string.Format("Entity of type \"{0}\" in state \"{1}\" has the following validation errors:", eve.Entry.Entity.GetType().Name, eve.Entry.State);
                    foreach (var ve in eve.ValidationErrors)
                    {
                        dd += string.Format("- Property: \"{0}\", Error: \"{1}\"", ve.PropertyName, ve.ErrorMessage);
                    }
                }
                return Content(dd);
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult sifretekrar(string ysfr)
        {
            ...
            if (user.sifre != ysfr)
            {
                ...
                Guid g4 = Guid.NewGuid();
                user.SecurityStamp = g4.ToString();
                user.PasswordHash = userManager.PasswordHasher.HashPassword(ysfr);
                db.Entry(user).State = EntityState.Modified;
                db.SaveChanges();
                return RedirectToAction("profil");
            }
        ...
        public ActionResult detay(string dty, int bnm)
        {
            return View(db.uruns.Where(a => a.Id == bnm).FirstOrDefault());
        }
        ...
        public ActionResult searching(string ara)
        {

            return View(db.uruns.Where(a => a.urunbaslik.ToLower().Contains(ara)).ToList());
        }
        ...
        public ActionResult OdemeSecimi()
        {
            return View();
        }
    }
